...
public class CommentedProperties extends java.util.Properties {
	 ...
	 */
	public void load(InputStream inStream, Map<String, String> mapIw) throws IOException
	{
		// The spec says that the file must be encoded using UTF_8.
		...
		while ((line = reader.readLine()) != null) {
			...
			if (!needsEscape) {
				if(mapIw.get(keyString) == null){
					put(keyString, "");
				}else{
					put(keyString, mapIw.get(keyString));
				}
		...
		for(Object key: mapIw.keySet()){
			if(!(keyData.contains(key) || key.equals("url"))){
				put(key, mapIw.get(key));
				lineData.add("");
				keyData.add("");
				lineData.add("");
				keyData.add(key);
	...
	public void store(OutputStream out, String header) throws IOException
	{
		...
		for (int i=0; i<keyData.size(); i++) {
			...
			if (key.length() > 0) {  // This is a 'property' line, so rebuild it
				...
				s.append ('=');
				formatForOutput (String.valueOf(get(key)), s, false);
