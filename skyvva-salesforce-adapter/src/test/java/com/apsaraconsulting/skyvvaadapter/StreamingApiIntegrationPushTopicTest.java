 ...
 */
class StreamingApiIntegrationPushTopicTest extends AbstractSalesforceTestBase {

    private static final String testPushTopicName = "RESTStreamingAPI";
    ...
    @Test
    void testCheckTestSObjectExistence() throws Exception {
        String sObjectAsJson = template().requestBody("direct:getSObject", idOfSObjectToUpdate, String.class);
        assertNotNull(sObjectAsJson);
        RESTStreamingAPI__c sObject = JsonUtils.createObjectMapper().readValue(sObjectAsJson, RESTStreamingAPI__c.class);
        assertEquals(idOfSObjectToUpdate, sObject.getId());
        assertEquals(nameOfSObjectToUpdate, sObject.getName());
    }

    @Test
    void testSubscribeAndReceiveFromPushTopic() throws Exception {

        MockEndpoint rawPayloadMock = getMockEndpoint(format("mock:RawPayload%s", testPushTopicName));
        rawPayloadMock.expectedMessageCount(1);
        // assert expected static headers
        rawPayloadMock.expectedHeaderReceived("CamelSalesforceTopicName", testPushTopicName);
        rawPayloadMock.expectedHeaderReceived("CamelSalesforceChannel", format("/topic/%s", testPushTopicName));
        ...
        assertNotNull(inRaw.getHeader("CamelSalesforceEventType"), "Missing header CamelSalesforceEventType");
        assertNotNull(inRaw.getHeader("CamelSalesforceCreatedDate"), "Missing header CamelSalesforceCreatedDate");
    ...
    protected RouteBuilder doCreateRouteBuilder() throws Exception {
        ...
        return new RouteBuilder() {
            ...
            fromF("salesforce:foo?topic=%s&loggingOption=%s&eventBodyExtractionStrategy=ALL_AS_BODY", testPushTopicName, loggingLevel).
                log(LoggingLevel.INFO, "Received message ${body}").
                toF("mock:RawPayload%s", testPushTopicName);
